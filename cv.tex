...
\ExplSyntaxOn
\makeatletter
\NewDocumentCommand \IfCensoredTF { m m } { \cs_if_eq:NNTF \censor \un@censor {#2} {#1} }
\makeatother

\cs_set_eq:NN \sean_old_href:nn \href
\cs_set_eq:NN \sean_old_url:n \url
\cs_set:Npn \href #1#2 {
  \exp_args:Nf \sean_old_href:nn { \IfCensoredTF {#1} { http://www.example.com } } {#2}
}
\cs_set:Npn \url #1 {
  \exp_args:Nf \sean_old_url:n { \IfCensoredTF {#1} { http://www.example.com } }
}
\ExplSyntaxOff
...
\begin{position}
  {
    ...
    date    = \daterange*{Jan 2015}{Present},
    city    = Verona,
    state   = WI,
    technical,
  }
...
\begin{position}
  {
    ...
    date    = \daterange{June 2014}{Jan 2015},
    city    = Fairfax,
    state   = VA,
    technical,
  }
...
\begin{position}
  {
    ...
    company = St. Mary's College,
    company-link = http://www.smcm.edu/mathcs,
    date    = \daterange{Sept 2013}{May 2014},
    city    = St. Mary's City,
    state   = MD,
    technical,
  }
...
I also maintain (or participate in) several open-source projects
  ...
  including \Lisp, Python, Java, and the C~languages.
(This does not include the many discussion-based contributions I've made.)

\project{
  title       = StackAPI,
  description = Language-native method stub and type generation using a scraped API,
  date        = \daterange*{2014}{Current},
...
\project{
  ...
  technical,
  technology  = C,
  date        = \daterange*{2012}{Current; Stagnant}
}
...
\project{
  ...
  date        = \daterange{March 28}{30}[2013],
}
